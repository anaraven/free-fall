red <- read.delim("~/Downloads/rep1/median-R.tsv", header=FALSE)
green <- read.delim("~/Downloads/rep1/median-G.tsv", header=FALSE)
blue <- read.delim("~/Downloads/rep1/median-B.tsv", header=FALSE)
...
r1 <- red[21:650,1]
r2 <-  sapply(-10:10, function(i) red[(i+20):(i+649),136])
plot(cor(r1,r2)[1,])

disp <- sapply(frames, function(k) {
    r2 <-  sapply(-10:10, function(i) red[(i+20):(i+649),k])
    ans <- cor(r1,r2)[1,]
